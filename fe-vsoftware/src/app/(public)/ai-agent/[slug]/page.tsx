...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  return {
    ...
    alternates: { canonical: `/ai-agent/${params.slug}` },
  }
}
    ...
    <>
      <BreadcrumbJsonLd
        ...
        items={[
          ...
          { label: 'Phần mềm AI Agent', href: '/ai-agent' },
          { label: agent.title, href: `/ai-agent/${agent.slug}` },
        ]}
      />
      <ServiceJsonLd
        name={agent.title}
      ...
      <section className="relative overflow-hidden">
        ...
        <div className="max-w-8xl mx-auto px-6 py-10 lg:py-14 min-h-[440px] flex flex-col justify-center">
          <nav className="text-[12px] text-vs-gray-600 mb-6">
            ...
            <Link href={`/${AI_AGENT_SLUGS}`} className="hover:text-vs-blue">Phần mềm AI Agent</Link>
            <span className="mx-2">/</span>
      ...
      {/* ── Breadcrumb ───────────────────────────────────────────────────── */}
      <div className="max-w-8xl mx-auto px-6 pt-3 pb-1">
        <Breadcrumb items={[
          { label: 'Trang chủ', href: '/' },
          { label: 'Phần mềm AI Agent', href: '/ai-agent' },
          { label: agent.title },
      ...
      {!com?.hidden && com && com.items.length > 0 && (
        ...
        <section className="py-14 bg-vs-bg">
          <div className="max-w-8xl mx-auto px-6">
            <h2 className="text-center text-[20px] sm:text-[24px] font-extrabold text-vs-dark mb-8">
              Các AI Agent khác của ViAI
            </h2>
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-5">
              {related.map((p) => (
                <Link key={p.id} href={`/${AI_AGENT_SLUGS}/${p.slug}`} className="bg-white rounded-2xl p-5 shadow-vs hover:shadow-vs-md hover:-translate-y-1 transition-all flex gap-4 no-underline">
