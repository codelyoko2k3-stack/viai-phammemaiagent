...
function WhySection({ config }: { config?: HomepageWhyConfig }) {
    ...
    const description = config?.description ?? 'ViAI thuộc hệ sinh thái ViTechGroup. Đội ngũ đã xây dựng và vận hành nhiều sản phẩm phần mềm thực tế — không phải team học việc.'
    ...
    const stats = config?.stats && config.stats.length > 0
        ...
        : WHY_STATS
    const quote = config?.quote ?? 'ViAI không chỉ viết code. Chúng tôi đồng hành cùng bạn xây dựng hệ thống — từ bài toán thực tế đến phần mềm vận hành thực sự.'
