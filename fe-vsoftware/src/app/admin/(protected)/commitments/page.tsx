  ...
  return (
    <div className="space-y-6 max-w-4xl">
      ...
      <AdminPageHeader
        ...
        description="5 cam kết hiển thị ở section &ldquo;ViAI cam kết&rdquo; trên các trang sản phẩm. Sửa 1 lần — áp dụng mọi nơi."
        showBack={false}
      >
        <button
          onClick={handleSave}
      ...
      {!config && !error ? (
        ...
        <div className="space-y-4">
          <SectionVisibilityToggle
            ...
            onChange={(h) => patch({ hidden: h })}
            sectionLabel="Cam kết"
            scopeLabel="toàn site (trang chủ + trang chi tiết AI Agent + Dịch vụ)"
          />

          <div className="rounded-2xl border border-slate-200 bg-white shadow-sm p-5 space-y-3">
            <Field label="Heading section (hiển thị trên web)">
              <input value={config.heading} onChange={(e) => patch({ heading: e.target.value })} className={inputCls} placeholder="VD: ViAI cam kết" />
            </Field>
          </div>
