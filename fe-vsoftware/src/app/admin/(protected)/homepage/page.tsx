...
const DEFAULT_CONFIG: HomepageConfig = {
  hero: {
    ...
    description:
      'Không dùng chung giải pháp đại trà. ViAI xây cho bạn đúng bài toán — CRM, App Mobile, Automation, Workflow — đúng chi phí, đúng tiến độ.',
  ...
  why: {
    ...
    description:
      'ViAI thuộc hệ sinh thái ViTechGroup. Đội ngũ đã xây dựng và vận hành nhiều sản phẩm phần mềm thực tế — không phải team học việc.',
    items: [
    ...
    quote: 'ViAI không chỉ viết code. Chúng tôi đồng hành cùng bạn xây dựng hệ thống — từ bài toán thực tế đến phần mềm vận hành thực sự.',
    ...
    tabs: [
      {
        ...
        title: 'CRM ViAI — Quản lý khách hàng & doanh số',
  ...
  seo: {
    // Khớp với nội dung SEO mặc định đang hiển thị live ở src/app/(public)/page.tsx
    // — để form admin hiện đúng nội dung thật khi DB chưa có dữ liệu, tránh đổi SEO ngoài ý muốn.
    title: 'ViAI — Phần mềm theo yêu cầu cho SMEs Việt Nam',
    description: 'ViAI xây dựng phần mềm quản lý theo yêu cầu cho doanh nghiệp vừa và nhỏ: CRM, App Mobile, AI Agent & Automation. Triển khai 4–8 tuần, bảo hành 12 tháng.',
    keywords: '',
    ogTitle: '',
    ogDescription: '',
    ogImage: '',
...
function WhyTab({
  ...
  return (
    <div className="space-y-4">
      <SectionVisibilityToggle hidden={why.hidden ?? false} onChange={(h) => set({ hidden: h })} sectionLabel="Why ViAI" />
      <SectionCard title="Tiêu đề section">
        <div className="grid grid-cols-2 gap-3">
...
function CharHint({ value, idealMin, idealMax }: { value?: string | null; idealMin: number; idealMax: number }) {
  const len = value?.length ?? 0
  const ok = len >= idealMin && len <= idealMax
  return (
    <span className={ok ? 'text-green-600 font-medium' : 'text-slate-400'}>
      {len}/{idealMin}-{idealMax} ký tự {ok && '— chuẩn SEO ✓'}
    </span>
  )
}
...
function SeoTab({
  ...
  function set(patch: Partial<HomepageSeoConfig>) {
    onChange({ ...config, seo: { ...seo, ...patch } })
  }

  return (
    <div className="space-y-4">
      <div className="rounded-lg border border-blue-200 bg-blue-50 px-4 py-3 text-[12.5px] text-blue-800 leading-relaxed">
        Tiêu đề, mô tả và thẻ OG inject vào <code className="bg-blue-100 px-1 rounded">&lt;head&gt;</code> trang chủ — ảnh hưởng trực tiếp đến kết quả Google và khi chia sẻ lên mạng xã hội.
      </div>

      <SectionCard title="SEO trên Google">
        <Field label="SEO Title" hint="Tiêu đề hiển thị trên Google + tab trình duyệt. Để trống → dùng tiêu đề mặc định.">
          <input
            value={seo.title ?? ''}
            onChange={(e) => set({ title: e.target.value })}
            className={inputCls}
            placeholder="VD: ViAI — Phần mềm AI Agent cho doanh nghiệp Việt Nam"
          />
          <p className="text-xs text-slate-400 mt-1"><CharHint value={seo.title} idealMin={50} idealMax={60} /></p>
        </Field>
        <Field label="Meta Description" hint="Dòng mô tả dưới tiêu đề trên Google. Nên có từ khóa + lời kêu gọi hành động.">
          <textarea
            ...
            className={inputCls}
          />
          <p className="text-xs text-slate-400 mt-1"><CharHint value={seo.description} idealMin={120} idealMax={160} /></p>
        </Field>
        <Field label="Keywords (từ khóa, phân cách bằng dấu phẩy)" hint="Chỉ lưu nội bộ — Google không còn đọc field này.">
          <input
            value={seo.keywords ?? ''}
            onChange={(e) => set({ keywords: e.target.value })}
            className={inputCls}
            placeholder="VD: AI Agent, tự động hóa doanh nghiệp, chatbot Việt Nam"
          />
        </Field>
      </SectionCard>

      <SectionCard title="Khi chia sẻ lên Facebook / Zalo (Open Graph)">
        <div className="grid grid-cols-2 gap-3">
          <Field label="OG Title" hint="Để trống → dùng SEO Title.">
            <input value={seo.ogTitle ?? ''} onChange={(e) => set({ ogTitle: e.target.value })} className={inputCls} />
          </Field>
          <Field label="OG Description" hint="Để trống → dùng Meta Description.">
            <input value={seo.ogDescription ?? ''} onChange={(e) => set({ ogDescription: e.target.value })} className={inputCls} />
          </Field>
        </div>
        <Field
          label="Logo SEO / Ảnh đại diện chia sẻ (OG Image)"
          hint="Ảnh thumbnail hiển thị khi link trang chủ được chia sẻ lên Facebook/Zalo. Tỉ lệ ngang 1.91:1 (vd 1200×630px). Để trống → dùng logo mặc định."
        >
          <MediaPicker
            value={{ src: seo.ogImage ?? '', alt: seo.ogImageAlt ?? '' }}
            onChange={(v) => set({ ogImage: v.src || null, ogImageAlt: v.alt || null })}
            minimal
          />
        </Field>
      </SectionCard>
...
export default function HomepageEditorPage() {
  ...
  useEffect(() => {
    adminGetHomepageConfig()
      // Gộp với DEFAULT_CONFIG để bù các mục bị thiếu trong dữ liệu cũ/không đầy đủ từ DB
      // (tránh crash "Cannot read properties of undefined" khi mở các tab tương ứng)
    ...
    <div className="space-y-6 max-w-4xl">
      ...
      {!config && !error ? (
        ...
        <>
          ...
          {activeTab === 'seo' && <SeoTab config={config} onChange={setConfig} />}
        </>
      ) : null}
    </div>
  )
