...
// Cấu hình mặc định — dùng để bù các mục bị thiếu khi dữ liệu từ DB không đầy đủ
// (tránh crash + tránh lưu đè làm mất field khi gọi API cập nhật)
const DEFAULT_CONFIG: FooterConfig = {
  brand: { title: 'ViAI', socials: [] },
  sections: [],
  copyright: '© {year} ViAI. All rights reserved.',
  legalLinks: [],
}
...
function BrandTab({ config, onChange }: { config: FooterConfig; onChange: (c: FooterConfig) => void }) {
  ...
  return (
    <div className="space-y-4">
      {/* Logo website */}
      <SectionCard title="Logo website">
        <Field
          label="Logo (hiển thị ở Header)"
          hint="Logo hiển thị ở góc trái thanh menu trên mọi trang. Để trống → dùng ảnh mặc định có sẵn. Khuyến nghị ảnh ngang, nền trong suốt (PNG), tối thiểu ~340×176px."
        >
          <MediaPicker
            value={{ src: brand.logoUrl ?? '', alt: brand.logoAlt ?? '' }}
            onChange={(v) => setBrand({ logoUrl: v.src || undefined, logoAlt: v.alt || undefined })}
            minimal
          />
        </Field>
      </SectionCard>
  ...
  useEffect(() => {
    adminGetFooterConfig()
      .then((res) => {
        ...
        const merged = { ...DEFAULT_CONFIG, ...raw, brand: { ...DEFAULT_CONFIG.brand, ...raw.brand } }
        if (!raw.sections && raw.columns) {
          setConfig({ ...merged, sections: raw.columns.map((c) => ({ type: 'links' as const, ...c })) })
        } else {
          setConfig(merged)
        }
      })
