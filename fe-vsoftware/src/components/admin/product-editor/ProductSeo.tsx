...
export default function ProductSeo({ values, onChange }: Props) {
  return (
    <div className="space-y-5 max-w-3xl">
      ...
      >
        <input value={values.seoTitle ?? ''} onChange={(e) => onChange({ seoTitle: e.target.value })} className={inputCls} placeholder="VD: AI Agent CSKH — Chatbot AI chăm sóc khách hàng 24/7 | ViAI" />
      </Field>
      ...
      <div className="rounded-xl bg-slate-50 border border-slate-200 p-4">
        ...
        <div className="text-[13px] text-slate-700 break-all">
          https://viai.vn/ai-agent/<span className="text-vs-blue font-semibold">{values.slug || 'slug-cua-san-pham'}</span>
        </div>
      </div>
    </div>
  )
