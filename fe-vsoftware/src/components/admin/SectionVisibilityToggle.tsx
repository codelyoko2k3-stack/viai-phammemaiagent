...
interface Props {
  ...
  sectionLabel?: string
  /** Phạm vi ảnh hưởng khi ẩn — mặc định 'trang chủ', có thể đổi cho section dùng chung nhiều trang */
  scopeLabel?: string
}

const SectionVisibilityToggle = ({ hidden, onChange, sectionLabel, scopeLabel = 'trang chủ' }: Props) => {
  return (
    <div
      ...
      <div className="flex items-center gap-3 min-w-0">
        ...
        <div className="min-w-0">
          <div className={`text-[13px] font-bold ${hidden ? 'text-amber-800' : 'text-emerald-800'}`}>
            {hidden ? `Section đang ẨN khỏi ${scopeLabel}` : `Section đang hiển thị trên ${scopeLabel}`}
          </div>
