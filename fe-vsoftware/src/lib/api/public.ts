...
export function getPosts(params: GetPostsParams = {}): Promise<PaginatedResponse<Post>> {
  ...
  return apiClient.get<BackendPaginatedResponse<Post>>(`/api/posts${qs ? `?${qs}` : ''}`, false, 60).then((res) => {
    if (res && res.meta) {
      return {
        data: res.data,
...
export function getPost(slug: string): Promise<ApiResponse<Post>> {
  return apiClient.get(`/api/posts/${slug}`, false, 60)
}
...
export function getCategoryPosts(
  ...
  if (params.limit) query.set('limit', String(params.limit))
  const qs = query.toString()
  return apiClient.get<BackendPaginatedResponse<Post>>(`/api/categories/${slug}/posts${qs ? `?${qs}` : ''}`, false, 60).then((res) => {
    if (res && res.meta) {
      return {
        data: res.data,
        total: res.meta.total,
