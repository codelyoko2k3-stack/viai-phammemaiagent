...
export interface HomepageSeoConfig {
  /** SEO Title — hiển thị trên kết quả tìm kiếm Google + tab trình duyệt. Để trống → dùng tiêu đề mặc định. */
  title?: string | null
  /** Meta Description — dòng mô tả dưới tiêu đề trên Google. */
  description?: string | null
  /** Keywords — chỉ lưu nội bộ, Google không còn đọc field này. */
  keywords?: string | null
  /** OG Title — tiêu đề khi chia sẻ link lên Facebook/Zalo. Để trống → dùng SEO Title. */
  ogTitle?: string | null
  /** OG Description — mô tả khi chia sẻ link lên Facebook/Zalo. Để trống → dùng Meta Description. */
  ogDescription?: string | null
  /** OG Image (logo SEO) — ảnh đại diện hiển thị khi chia sẻ link lên Facebook/Zalo. Để trống → dùng logo mặc định. */
  ogImage?: string | null
...
export interface FooterConfig {
  brand: {
    /** Tiêu đề cột Brand mới (vd "ViAI"). Render giống các cột links khác. */
    title?: string
    /** Danh sách MXH (vẫn dùng cho cột Brand) */
    socials: FooterSocial[]
    /** Logo website — hiển thị ở Header trên mọi trang. Để trống → dùng ảnh mặc định /logo-ngang.png */
    logoUrl?: string
    logoAlt?: string
    // ─── Field cũ (giữ optional cho backward compat — không còn dùng) ───
    tagline?: string
